...
export const updateProduct = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const { productName, categoryName, options, variants, status, description, sku, deletedImage } =
    req.body;
  const variantParsed = JSON.parse(variants);
  const optionParsed = JSON.parse(options);
  try {
    ...
    if (!findProduct || !findCategory) {
      if (req.files) {
        await Promise.all(req.files.map((data) => cloudinary.uploader.destroy(data.filename)));
      }
...
export const deleteProduct = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    if (findImage.length > 0) {
      await Promise.all(findImage.map((image) => cloudinary.uploader.destroy(image.cloudinaryId)));
